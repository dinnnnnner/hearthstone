...
test('choose each AI model, keep it on reconnect, and share the choice in friend rooms', async ({ page, request }) => {
  ...
  expect(options.map((m: { label: string }) => m.label)).toEqual(['64 层模型', '256 层模型', '1024 层模型']);
  try {
    ...
    for (const model of options) {
      await page.getByRole('radio', { name: new RegExp('^' + model.label) }).check();
      ...
      await api('/leave', {});
      await page.reload();
      await expect(page.getByRole('radio', { name: new RegExp('^' + model.label) })).toBeChecked();
    }
    ...
    await expect(page.locator('.room-heading')).toContainText('1024 层模型');
