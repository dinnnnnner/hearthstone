...
手机小屏默认使用手游模式。横屏时酒馆、战场与手牌同屏展示，英雄技能与结束招募位于右侧。竖屏也可游玩；酒馆及手牌可以左右滑动。

点选卡牌查看技能，再点击招募、打出、发动或出售。点选战场随从后可用左右移动按钮调整站位，无需拖拽。黑暗发现和已装备饰品位于“赛季玩法”中。

顶部的手机/显示器按钮可切换布局；在偏好设置中可选择跟随屏幕、手游模式或标准模式。布局偏好单独保存，切换及旋转屏幕不会重开对局。全屏按钮在浏览器支持时进入全屏，也适配屏幕安全区域。这是手机网页模式，不是独立 Android/iOS 安装包。
