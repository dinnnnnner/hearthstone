...
房间服务使用独立的 `bobs-tavern` 系统用户，监听回环地址 8787。`/var/lib/bobs-tavern/state.json` 每秒保存有变更的状态，正常停止时再保存一次。文件权限为 0600，包含游客令牌的哈希、房间和对局快照；不要放入静态站点、源码仓库或公开备份。浏览器只保存自己的随机游客令牌，不接收其他玩家的手牌与酒馆。

`systemctl status bobs-tavern` 查看状态，`journalctl -u bobs-tavern` 查看服务日志。服务重启会恢复游客和房间；突然断电最多可能丢失尚未落盘的最近一秒变更。发布脚本保存上一后端链接到新目录的 `rollback.json`，失败会恢复旧服务；后端成功但前端发布失败时，前端脚本会恢复原站点，可在排查后重新发布前端。
...
为限制小服务器开销，等待房间和进行中的房间总数最多 24，同时进行最多 6 局；长战斗只发送前 179 个快照和最终快照，规则结算完整执行。对局最多 50 回合，届时按剩余生命与护甲排序，同值按座位顺序。完成的房间空闲 10 分钟、其他房间空闲 2 小时回收；无房间的游客 30 天未访问后清理。
