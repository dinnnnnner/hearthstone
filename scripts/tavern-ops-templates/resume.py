...
command=['/root/miniconda3/bin/python','-u','/root/tavern-human-current-06cd2177/population_resume.py','--resumes',*[str(root/f'member-{i}/training/latest.pt') for i in range(3)],'--output',str(root),'--deadline-utc',end.isoformat(),'--workers','16','--iterations-per-exchange','2','--games-per-iteration','16','--max-per-lineage','2','--external-fraction','0.4','--unused-gold-penalty','0.01','--device','cuda']
