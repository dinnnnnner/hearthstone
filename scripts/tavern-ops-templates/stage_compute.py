...
reports=json.loads((incoming/'report.json').read_text())['models'];rollback={};results=[]
for m in reports:
 ...
 for name,digest in m['hashes'].items():
  src=incoming/'export'/ident/name;assert hashlib.sha256(src.read_bytes()).hexdigest()==digest;shutil.copy2(src,new/name)
 hashes=json.loads((new/'hashes.json').read_text());hashes.update(m['hashes']);(new/'hashes.json').write_text(json.dumps(hashes,indent=2)+'\n')
 for file in (old/'python').rglob('*.py'):
  assert file.read_bytes()==(new/file.relative_to(old)).read_bytes()
 env=dict(os.environ,PYTHONPATH=str(new/'python'),OMP_NUM_THREADS='1',OPENBLAS_NUM_THREADS='1',MKL_NUM_THREADS='1')
