...
2026-09-11 更新已将当前卡牌、排名、配对规则及公开战况接入软件包。观察版本升为 4，实体版本升为 3，实体数仍为 73，MLP 输入由 2705 维增至 3209 维。每个玩家实体提供最近两回合的阵容类型、种族数量、交战座位、胜负和伤害，以及固定座位、排名生命值、法术护甲。八个玩家的座位编号为 0～7，战况中的幽灵为 8；空场、无种族和混合阵容单独编码，混合阵容不猜测种族数量。缺失战况不当作平局，本回合、未来及两回合以前的战况不进入输入。

新包使用新的规则及源码哈希，不接受旧规则检查点直接续训，也不接受旧模型作为冻结对手。旧包和模型保留在原目录；本页末尾的历史续训命令仅适用于当时的软件包。新增输入已经接通，不代表已有模型已经学会使用它。交付位置及验证见 [本次更新记录](rl-public-scouting.md)。
