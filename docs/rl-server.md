...
续训请使用随包提供的 `rl-dist/bridge.cjs`。这份模拟器从 `686fb73` 的独立代码副本构建，版本哈希为 `b6903488d2ee12d2113c84941d5e5fdb357bf36b1b681110c019b0697eaa62d3`。v2 当时的构建哈希还包含部分界面 TypeScript 文件；v3 已改成只追踪模拟器实际依赖，界面变化不再改变规则指纹。不要覆盖这份已验证模拟器，或修改检查点哈希绕过兼容性校验。
