...
大厅的 [AI 观战](ai-spectator.md) 可以查看模型对买牌、升本、刷新等操作的偏好，并暂停或逐步观看。

## 2026-09-17 发布

三个普通模型和对应搜索版已切换到当前场面与经济反馈实验的最后保存检查点，64／256／1024 层分别为 4424／4936／5136 局。这些是 `basic-feedback` 实验独立计数，不包含热启动之前的局数。本次普通版采用 PPO 策略抽样，搜索版使用相同权重继续搜索，观战仍显示策略偏好。

发布版本为 `20260917-basic-feedback`，保留上一版本供回滚。网页和房间服务代码未改动。发布时复查实际进程发现训练已停止，训练状态文件最后更新于北京时间 07:09:56，文件内的 `running` 已过期。本次只发布已保存检查点，没有重新启动训练。验证记录见 [发布记录](model-publication-20260917.json)。

## 模型注册与网页选择

以下是 2026-09-16 新架构发布时 `deploy/inference-models.json` 的快照。后续上线以注册表及 `status` 输出为准。

| 选项 | 模型标识 | 已发布局数 | 公网回环端口 | 计算端口 |
| --- | --- | ---: | ---: | ---: |
| 64 层 | `deep64-1480` | 7971 | 18891 | 18890 |
| 256 层 | `deep256-1164` | 8101 | 18893 | 18892 |
| 1024 层 | `deep1024-612` | 7778 | 18895 | 18894 |
| 64 层加搜索 | `deep64-search-2568` | 7971 | 18897 | 18896 |
| 256 层加搜索 | `deep256-search-2140` | 8101 | 18901 | 18900 |
| 1024 层加搜索 | `deep1024-search-1284` | 7778 | 18903 | 18902 |

网页以“新版”标注这些模型，包含动作估值、卡牌估值、场面评估及 1、3、5 回合预测。普通模型的 ID 保留了早期名称，末尾数字不是当前局数。判断版本应读 `episodes` 和 `checkpointSha256`。搜索选项使用独立服务，本次六个选项同步更新。日常 `publish` 默认更新三个普通模型，带 `--include-search` 时同步搜索版本。
