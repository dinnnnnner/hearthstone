...
当前训练接口为 v2，输入 2705 维，动作 4316 个。v2 补齐克罗米整排酒馆法术的 7 个位置，v1 检查点和动作记录与它不兼容。旧记录应保留，不能直接改版本号强行续训。`docs/rl-validation.json` 保存 v1 的本地验证；v2 的 GPU 验证见 `docs/rl-server-validation.json`。
