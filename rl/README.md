...
当前默认是 v3：73 个卡牌和状态实体、Transformer + GRU、4316 个分层动作，以及按玩家连续序列更新的 PPO。完整说明和固定评估命令见 [v3 说明](../docs/rl-v3.md)。`--architecture mlp` 保留 2705 维旧观察，用于对照。v1、v2 检查点不可直接用于 v3 续训；构建验证通过的指定 v2 模型可以作为冻结对手。历史 v2 GPU 验证仍保存在 `docs/rl-server-validation.json`，不代表新网络的测试结果。
...
小规模测试只能证明流程可运行，不能证明棋力。长期训练前应继续补规则、测量当前网络吞吐，并用固定评估套件检验进步。训练产物尚未接入网站人机。
