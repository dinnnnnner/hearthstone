...
2026-09-16 新增独立战斗模拟监督的场面评估头，预测当前及不同动作之后的战力，并接入动作Q。见 [场面战力评估](../docs/rl-scene-value.md)。

2026-09-16 新增统一动作估值训练：逐牌金币估值与动作回报分开，买卖、刷新、升本直接按 Q 比较。启用 `--action-value` 后用收益回归替代 PPO，详见 [按动作价值决策](../docs/rl-action-value.md)。
...
2026-09-16 新增 `--streaming` 持续训练：对局没结束也能更新 PPO，预测战斗和金币、卡牌储备、折扣等经济资源，并从同局面比较不同买卖选择。它继承现有模型，替换旧金币规划训练模式。预算、恢复命令和估值限制见 [训练流程](../docs/rl-training.md)。
...
新包默认只允许一次收尾冻结或解冻；冻结要求存在当前金币买不起的牌。每回合换位最多 6 次，并限制立即撤销。旧检查点需要明确的规则迁移，不能直接替换其冻结模拟器，见 [动作限制](../docs/ai-action-limits.md)。
...
多时间尺度训练使用 1、3、5 回合后继预测和按回合匹配的参照对手，见 [多时间尺度场面与经济预测](../docs/rl-multi-horizon.md)。
